...
export default function HUAuditUpdateSeals(clientAPI) {
    ...
    if (tu._HandlingUnitAudit.quantity >= cantGC) {
        ...
        let sello1 = clientAPI.evaluateTargetPath('#Page:Import_Inspection_HU_Seals/#Control:Seal1/#Value');
        let sello2 = clientAPI.evaluateTargetPath('#Page:Import_Inspection_HU_Seals/#Control:Seal2/#Value');
        let nuevoSello1 = clientAPI.evaluateTargetPath('#Page:Import_Inspection_HU_Seals/#Control:NewSeal1/#Value');
        let nuevoSello2 = clientAPI.evaluateTargetPath('#Page:Import_Inspection_HU_Seals/#Control:NewSeal2/#Value');
        return clientAPI.executeAction({
            "Name": "/MDK01/Actions/Import_Inspection/HUAuditUpdate.action",
            "Properties" : {
                "Properties" : {
                    "sello1": `${sello1}`,
                    "sello2": `${sello2}`,
                    ...
                    if (listPickerValue && listPickerValue.length === 0) {
                        ...
                        return clientAPI.read('/MDK01/Services/API01.service', 'stockKeepingUnit',[], query).then((result) => {
                            if (result !== undefined && result._array.length > 0) {   
                                sku = result._array[0]; 
                                return libSam.setMilitaryStd(clientAPI,sku,tu,cantBC,libCom.getStateVariable(clientAPI, 'Import_Inspection_Sample'),milStdReadSuccessful ,milStdReadError);
                            } else {
                                return addQuantity(clientAPI,cantBC,tu);     
                            }
                        })                          
                    } else {
                        sku = clientAPI.evaluateTargetPathForAPI('#Page:Import_Inspection_Add_HU_Detail').getControl('FormCellContainer').getSections()[2].binding._array[0];
                        return libSam.setMilitaryStd(clientAPI,sku,tu,cantBC,libCom.getStateVariable(clientAPI, 'Import_Inspection_Sample'),milStdReadSuccessful ,milStdReadError);                      
                    }
                } else {
                    return addQuantity(clientAPI,cantBC,tu);         
...
export function milStdReadError(clientAPI,tu) {
    console.log("MDK01 Import_Inspection_HU_SealsWSealsBtnPress error fetching military standards");
...
export function addQuantity(clientAPI,cantBC,tu) {
    let quantity = libCom.getStateVariable(clientAPI, 'Import_Inspection_SampleCount');
    quantity = quantity + tu._HandlingUnitAudit.quantity;
    //let quantity = tu._HandlingUnitAudit.quantity;
    let prevCantBC = libCom.getStateVariable(clientAPI, 'Import_Inspection_SampleRejects' );
    cantBC = cantBC + prevCantBC;                
    libCom.setStateVariable(clientAPI, 'Import_Inspection_SampleCount', quantity );
