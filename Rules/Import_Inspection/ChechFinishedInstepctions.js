...
export default function CheckFinishedInspections(clientAPI) {
    ...
    if (selectedItems.length > 0) {
        let pageProxy = clientAPI.evaluateTargetPathForAPI('#Page:Import_Inspection_Add_HU');
        let pageBinding = pageProxy.getBindingObject();
        let completedAudits = pageBinding._HandlingUnitAudit;
        let listPicker = clientAPI.evaluateTargetPath('#Page:Import_Inspection_Add_HU/#Control:HULstPkr');
