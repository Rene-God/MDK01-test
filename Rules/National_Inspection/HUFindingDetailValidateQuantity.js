...
export default function HUFindingDetailValidateQuantity(clientAPI) {
    ...
    var cantGCField = clientAPI.evaluateTargetPath('#Page:National_Inspection_Add_HU_Detail/#Control:PiecesGC');
    ...
    if (listPickerValue && listPickerValue.length > 0) {
        if (tu._HandlingUnitAudit.quantity >= cantGC) {
            return clientAPI.executeAction("/MDK01/Actions/National_Inspection/NavToHUInspectionSeals.action");
        } else {
