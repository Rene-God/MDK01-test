import National_InspectionAuditListFilter from "./National_InspectionAuditListFilter";
...
export default function Get_Import_InspectionResultValidation(clientAPI) {
    ...
    console.log("MDK01 Get_National_InspectionResultValidation");
    if (clientAPI.actionResults.Get_National_Inspection.data === undefined || clientAPI.actionResults.Get_National_Inspection.data._array.length === 0) {
        ...
        var query = National_InspectionAuditListFilter(clientAPI);
        var appointment = clientAPI.evaluateTargetPath('#Page:National_Inspection_Create/#Control:FCIIAppointment').getValue();
            ...
            if (result === undefined || result._array.length === 0) {
                ...
                return clientAPI.read('/MDK01/Services/API05.service', 'TU_AUDIT', [], query).then((result) => {
                    if (result === undefined || result._array.length === 0 ) {
                        ...
                        return clientAPI.executeAction("/MDK01/Actions/National_Inspection/National_InspectionHeaderCreate.action");
                    }
                });                      
            } else {
