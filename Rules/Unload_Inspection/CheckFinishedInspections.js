...
export default function CheckFinishedInspections(clientAPI) {
    ...
    if (selectedItems.length > 0) {
        let pageProxy = clientAPI.evaluateTargetPathForAPI('#Page:Unload_Inspection_Add_HU_Finding');
        let pageBinding = pageProxy.getBindingObject();
        let completedAudits = pageBinding._HandlingUnitAudit;
        let listPicker = clientAPI.evaluateTargetPath('#Page:Unload_Inspection_Add_HU_Finding/#Control:HULstPkr');
